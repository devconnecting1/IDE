    ...
    <div className="overflow-hidden rounded-xl bg-card ring-1 ring-foreground/10">
      <div className="grid grid-cols-1 xl:grid-cols-12">
        <Card className="gap-5 overflow-hidden rounded-none border-0 border-foreground/10 border-b ring-0 xl:col-span-4 xl:border-r xl:border-b-0">
          <CardHeader>
            <CardTitle className="font-normal">{t("txnKpiInflow")}</CardTitle>
          </CardHeader>
          <CardContent className="flex items-end justify-between">
            <div className="space-y-1">
              ...
              <p className="text-muted-foreground text-xs">
                {t("txnKpiCredits", { count: monthRows.filter((row) => row.amount > 0).length })}
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="gap-5 overflow-hidden rounded-none border-0 border-foreground/10 border-b ring-0 xl:col-span-4 xl:border-r xl:border-b-0">
          <CardHeader>
            <CardTitle className="font-normal">{t("txnKpiOutflow")}</CardTitle>
          </CardHeader>
          <CardContent className="flex items-end justify-between">
            <div className="space-y-1">
              <div className="text-3xl tabular-nums leading-none tracking-tight">
              ...
              <p className="text-muted-foreground text-xs">
                {t("txnKpiDebits", { count: monthRows.filter((row) => row.amount < 0).length })}
              </p>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="gap-5 overflow-hidden rounded-none border-0 ring-0 xl:col-span-4">
          <CardHeader>
            <CardTitle className="font-normal">{t("txnKpiNet")}</CardTitle>
          </CardHeader>
          <CardContent className="flex items-end justify-between">
            <div className="space-y-1">
              <div
              ...
              <p className="text-muted-foreground text-xs">{t("txnKpiToDate", { count: monthRows.length })}</p>
            </div>
          </CardContent>
        </Card>
      </div>
