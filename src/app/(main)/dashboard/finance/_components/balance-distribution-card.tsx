...
const currencies = {
  EUR: { key: "currencyEuro" as const },
  GBP: { key: "currencyGbp" as const },
  USD: { key: "currencyUsd" as const },
} as const;
...
export function BalanceDistributionCard() {
  ...
  const balanceData = React.useMemo(
    () => [
      { account: t("accountMain"), amount: 122_540, key: "main" as BalanceKey, percentage: 52.2 },
      { account: t("accountSavings"), amount: 48_320, key: "savings" as BalanceKey, percentage: 20.6 },
      { account: t("accountInvestment"), amount: 36_780, key: "investment" as BalanceKey, percentage: 15.7 },
      { account: t("accountReserve"), amount: 27_256, key: "reserve" as BalanceKey, percentage: 11.5 },
    ],
    [t],
  );
  ...
  const chartConfig = React.useMemo<ChartConfig>(
    () => ({
      amount: { label: t("total") },
      investment: { color: "var(--chart-1)", label: t("accountInvestment") },
      main: { color: "var(--chart-2)", label: t("accountMain") },
      reserve: { color: "var(--chart-3)", label: t("accountReserve") },
      savings: { color: "var(--chart-4)", label: t("accountSavings") },
    }),
    [t],
  );

  const totalBalance = React.useMemo(() => balanceData.reduce((total, item) => total + item.amount, 0), [balanceData]);
