...
function formatMonthlyDelta(t: (key: string, params?: Record<string, string | number>) => string, delta: number) {
  if (delta === 0) return t("accountsFlatThisMonth");
  const sign = delta > 0 ? "+" : "";
  return t("accountsDeltaThisMonth", { sign, amount: formatCurrency(delta, { noDecimals: true }) });
}
...
export function AccountsList({ onSelectAccount }: AccountsListProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="font-normal text-lg">{t("accountsAllAccounts")}</CardTitle>
        <CardDescription>{t("accountsTapRow")}</CardDescription>
        <CardAction className="hidden items-baseline gap-2 sm:flex">
          ...
          <span className="text-muted-foreground text-xs">{t("accountsAcrossCount", { count: accounts.length })}</span>
        </CardAction>
      </CardHeader>
                ...
                {group.accounts.map((account) => {
                  ...
                  const trimmedDetail =
                    account.type === "crypto" && account.cryptoBalance
                      ...
                      : account.detail;
                  const deltaText = formatMonthlyDelta(t, account.monthlyDelta);
