...
const dateFnsLocales = { "pt-BR": ptBR, en: enUS } as const;
...
export default function Page() {
  ...
  const formattedDate = now
    ? format(now, "EEEE, do MMMM yyyy", { locale: dateFnsLocales[locale as keyof typeof dateFnsLocales] ?? enUS })
    : " ";
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col gap-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <TabsList variant="line">
            <TabsTrigger value={DASHBOARD_TAB}>{t("tabDashboard")}</TabsTrigger>
            <TabsTrigger value={ACCOUNTS_TAB}>{t("tabAccounts")}</TabsTrigger>
